...
FIX: Correct Workday job URL construction so apply links go directly
     to the job posting page (not the Workday maintenance/community page).

Workday URL format:
  https://{company}.wd{N}.myworkdayjobs.com/{TenantPath}/job/{location-slug}/{title-slug}_{jobId}
  The `externalPath` from the API already contains the full path after the tenant,
  e.g.  /job/Bengaluru/Software-Engineer_JR-123456
  So the correct apply URL = WORKDAY_BASE + externalPath

Each company has a verified WORKDAY_BASE defined in WORKDAY_TENANTS below.
"""
...
# Each entry: (api_jobs_url, base_url_for_apply_links, fallback_careers_url)
#
# HOW apply links work:
#   The Workday API returns externalPath like:
#     /job/Bengaluru/Associate-Software-Engineer_JR-0123456
#   The correct apply URL is:
#     base_url + externalPath
#   e.g.:
#     https://accenture.wd3.myworkdayjobs.com/Accenture_Careers + /job/Bengaluru/...
#     → https://accenture.wd3.myworkdayjobs.com/Accenture_Careers/job/Bengaluru/...
#
# To find the correct tenant path: go to the company careers page,
# click any job, and note the URL structure.
# ─────────────────────────────────────────────────────────────
WORKDAY_TENANTS = {
    'accenture': {
        'api_url':      'https://accenture.wd3.myworkdayjobs.com/wday/cxs/accenture/Accenture_Careers/jobs',
        'apply_base':   'https://accenture.wd3.myworkdayjobs.com/Accenture_Careers',
        'fallback_url': 'https://www.accenture.com/in-en/careers',
    },
    'tcs': {
        'api_url':      'https://tcs.wd3.myworkdayjobs.com/wday/cxs/tcs/TCS/jobs',
        'apply_base':   'https://tcs.wd3.myworkdayjobs.com/TCS',
        'fallback_url': 'https://www.tcs.com/careers',
    },
    'wipro': {
        'api_url':      'https://wipro.wd3.myworkdayjobs.com/wday/cxs/wipro/WEXT/jobs',
        'apply_base':   'https://wipro.wd3.myworkdayjobs.com/WEXT',
        'fallback_url': 'https://careers.wipro.com',
    },
    'cognizant': {
        'api_url':      'https://cognizant.wd3.myworkdayjobs.com/wday/cxs/cognizant/Cognizant/jobs',
        'apply_base':   'https://cognizant.wd3.myworkdayjobs.com/Cognizant',
        'fallback_url': 'https://www.cognizant.com/careers',
    },
    'infosys': {
        'api_url':      'https://infosys.wd3.myworkdayjobs.com/wday/cxs/infosys/Infosys/jobs',
        'apply_base':   'https://infosys.wd3.myworkdayjobs.com/Infosys',
        'fallback_url': 'https://www.infosys.com/careers',
    },
    'ibm': {
        'api_url':      'https://ibm.wd3.myworkdayjobs.com/wday/cxs/ibm/IBMExternalJobBoard/jobs',
        'apply_base':   'https://ibm.wd3.myworkdayjobs.com/IBMExternalJobBoard',
        'fallback_url': 'https://www.ibm.com/careers',
    },
    'techmahindra': {
        'api_url':      'https://careers.techmahindra.com/wday/cxs/techmahindra/TechMahindra/jobs',
        'apply_base':   'https://careers.techmahindra.com/TechMahindra',
        'fallback_url': 'https://careers.techmahindra.com',
    },
    'ltimindtree': {
        'api_url':      'https://ltimindtree.wd3.myworkdayjobs.com/wday/cxs/ltimindtree/LTIMindtreeCareers/jobs',
        'apply_base':   'https://ltimindtree.wd3.myworkdayjobs.com/LTIMindtreeCareers',
        'fallback_url': 'https://www.ltimindtree.com/careers',
    },
}


def build_workday_apply_url(tenant_key: str, external_path: str) -> str:
    """
    Construct a valid Workday job posting URL.

    external_path from API looks like:
      /job/Bengaluru/Associate-Software-Engineer_JR-0123456
    
    Final URL:
      https://accenture.wd3.myworkdayjobs.com/Accenture_Careers/job/Bengaluru/Associate-Software-Engineer_JR-0123456
    """
    tenant = WORKDAY_TENANTS.get(tenant_key, {})
    apply_base = tenant.get('apply_base', '')
    fallback = tenant.get('fallback_url', 'https://www.google.com/about/careers')

    if not external_path or not apply_base:
        return fallback

    # Ensure no double slashes
    path = external_path if external_path.startswith('/') else '/' + external_path
    return apply_base.rstrip('/') + path


def build_workday_search_url(tenant_key: str, title: str) -> str:
    """Fallback: link to careers search page filtered by job title."""
    tenant = WORKDAY_TENANTS.get(tenant_key, {})
    apply_base = tenant.get('apply_base', '')
    if apply_base:
        return f"{apply_base}?q={quote_plus(title)}"
    return tenant.get('fallback_url', '')
...
def is_today(date_str: Optional[str]) -> bool:
    """Check if a date string represents today or yesterday."""
    if not date_str:
        return True
    try:
        ds = str(date_str)[:10]
...
# Used by: Accenture, TCS, Wipro, Cognizant, Infosys, IBM, Tech Mahindra, LTIMindtree
# ─────────────────────────────────────────────────────────────
...
def scrape_workday(
    ...
    Uses the CXS (JSON) API endpoint — this is the correct internal API
    that Workday career sites use. The standard /jobs endpoint returns HTML;
    the /wday/cxs/.../jobs endpoint returns JSON with jobPostings.

    apply_url is constructed via build_workday_apply_url() to get a
    direct link to the job posting page.
    """
    tenant = WORKDAY_TENANTS[tenant_key]
    api_url      = tenant['api_url']
    fallback_url = tenant['fallback_url']

    log.info(f"Scraping {company_name} via Workday CXS API...")
    jobs = []

    try:
        # Workday CXS API: POST with JSON body for pagination
        payload = {
            "appliedFacets": {},
            "limit": min(limit, 20),
            "offset": 0,
        ...
        if resp.status_code == 200:
            ...
            log.info(f"{company_name} Workday CXS: {len(items)} items returned")
            ...
            for item in items[:limit]:
                ...
                # ✅ KEY FIX: build correct apply URL from externalPath
                if external_path:
                    apply_url = build_workday_apply_url(tenant_key, external_path)
                else:
                    apply_url = build_workday_search_url(tenant_key, title)
            ...
            log.warning(f"{company_name} Workday CXS returned status {resp.status_code}")

    except Exception as e:
        log.error(f"{company_name} Workday error: {e}")

    # ── Fallback: use default roles with search URLs ──────────
    if not jobs:
        log.info(f"{company_name}: Using fallback default roles")
        for role in default_roles[:limit]:
    ...
    log.info(f"{company_name} total: {len(jobs)}")
    return jobs
...
def scrape_wipro(limit: int) -> list:
    return scrape_workday(
        ...
        limit=limit,
        default_roles=[
            ("Senior Systems Software Engineer", "5+ years"),
            ("Software Engineer", "0-2 years"),
            ("IT Infrastructure Engineer", "2-4 years"),
            ("Cloud Solutions Specialist", "2-4 years"),
...
def scrape_cognizant(limit: int) -> list:
    return scrape_workday(
        tenant_key='cognizant',
        company_name='Cognizant',
        limit=limit,
        default_roles=[
            ("Senior Consultant", "5+ years"),
            ("Consultant", "2-4 years"),
            ("Associate Consultant", "Fresher"),
            ("Technology Engineer", "0-2 years"),
...
# ── CAPGEMINI (SmartRecruiters — different ATS) ───────────────
def scrape_capgemini(limit: int) -> list:
    """
    Capgemini uses SmartRecruiters ATS.
    Apply URL format: https://jobs.smartrecruiters.com/Capgemini/{jobId}
    """
    log.info("Scraping Capgemini (SmartRecruiters)...")
    jobs = []
    try:
        ...
        if resp.status_code == 200:
            ...
            for item in items[:limit]:
                ...
                if not title:
                    continue
                job_id = item.get('id', '')
                # ✅ Direct SmartRecruiters job posting URL
                apply_url = (
                    f"https://jobs.smartrecruiters.com/Capgemini/{job_id}"
                    if job_id else
                    'https://www.capgemini.com/careers/job-search/'
                )
                ...
                jobs.append({
                    ...
                    'company': 'Capgemini',
                    'location': loc[:200],
                    'work_mode': detect_work_mode(title + loc),
                    'job_type': 'Full-time',
                    'experience': detect_experience(title),
                    'salary_text': None,
    ...
    if not jobs:
        log.info("Capgemini: Using fallback default roles")
        default_roles = [
            ("Senior Consultant", "5+ years"),
            ("Consultant", "2-4 years"),
            ("Associate Consultant", "Fresher"),
        ...
        for title, exp in default_roles[:limit]:
            jobs.append({
                ...
                'apply_url': f"https://www.capgemini.com/careers/job-search/?search={quote_plus(title)}&country=India",
                'apply_source': 'Company',
                'category': map_category(title),
                'is_featured': False,
            })
...
def scrape_deloitte(limit: int) -> list:
    """
    Deloitte India careers — HTML scrape.
    apply_url: direct link extracted from job card anchor tags.
    """
    log.info("Scraping Deloitte...")
    jobs = []
    try:
        url = (
            "https://apply.deloitte.com/careers/SearchJobs"
            "?Location=India&LocationRadius=0&listFilterMode=1&sortBy=relevance&ascending=0"
        )
        ...
        log.info(f"Deloitte: {len(cards)} cards found")
        for card in cards[:limit]:
            ...
            if href and href.startswith('/'):
                ...
                href = f"https://apply.deloitte.com/careers/SearchJobs?searchKeyword={quote_plus(title_el.get_text(strip=True))}&Location=India"
    ...
    if not jobs:
        log.info("Deloitte: Using fallback default roles")
        default_roles = [
            ("Senior Consultant", "5+ years"),
            ("Consultant", "2-4 years"),
            ("Associate Consultant", "Fresher"),
